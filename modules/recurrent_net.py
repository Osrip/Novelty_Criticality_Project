...
class RecurrentNet():
    '''
    Network class that implements a recurrent neural network
    ...
    activation: function, activation function for the neurons
    n_internal_steps: int, number of internal passes
    ...
    activate(self, inputs)
        Calculates the value of the network output given the input

    create(genome, config, batch_size=1, activation=sigmoid_activation,
               prune_empty=False, use_current_activs=False, n_internal_steps=1)
        Create a RecurrentNet object (static method)
    '''
    ...
    def activate(self, inputs):
        '''
        Calculates the value of the network output given the input
        :param inputs: Inputs for sensory input neurons
        :return: outputs
        '''
        with torch.no_grad():
            ...
            if self.n_hidden > 0:
                # Iterating through internal forward passes
